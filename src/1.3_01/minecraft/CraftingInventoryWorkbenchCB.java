...
// === */
        ...
        if(dir != 0){
		    func_20117_a(new SlotCraftingMultiple(craftMatrix, craftResult, 0, 124, 35));
		}else{
		    func_20117_a(new SlotCrafting(craftMatrix, craftResult, 0, 124, 35));
		}
// >>>        	
...
class SlotCraftingMultiple extends Slot
{
    ...
    public SlotCraftingMultiple(InventoryCrafting iinventory, IInventory iinventory1, int i, int j, int k)
    {
    ...
    public void onPickupFromSlot()
    {
	    ...
	    ItemStack outputstack = CraftingManager.getInstance().findMatchingRecipe(craftMatrix);
    	if(outputstack != null){
			if (outputstack.getMaxStackSize() < total){
				total = outputstack.getMaxStackSize();
			}
    	}
